...
async def read_items(container_obj, items_to_read):
    for family in items_to_read:
        item_answer = await container_obj.read_item(item=family['id'], partition_key=family['lastName'])
        request_charge = container_obj.client_connection.last_answer_headers['x-ms-request-charge']
        print('Read item with id {0}. Operation consumed {1} request units'.format(item_answer['id'], (request_charge)))

async def query_items(container_obj, query_text):
    query_items_answer = container_obj.query_items(
        query=query_text
    )
...
async def read(container_name, query):
    async with cosmos_client(endpoint, credential = key) as client:
        try:
            ...
            raise Exception(f"Ocurrio un error al momento de buscar en {container_name}")
...
async def create_update(container_name, body):
    async with cosmos_client(endpoint, credential = key) as client:
        try:
            ...
            logging.error(e.message)
            raise Exception(f"Ocurrio un error al momento de actualizar en {container_name}")
...
async def delete(container_name, doc_id, partition_key):
    async with cosmos_client(endpoint, credential = key) as client:
        try:
            ...
            logging.error(e.message)
            raise Exception(f"Ocurrio un error al momento de eliminar en {container_name}")